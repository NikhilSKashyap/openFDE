...
# ── Centralized role instructions for the three brief sections ────────────────
# ONE source of truth for "who owns which section + what their job is", deliberately short and
# practical (not long personas). Easy to make user-customizable later by moving these into
# agent_settings. The brief uses the short alias keys (architect / sr_dev / verifier).
SECTION_ROLE_PROMPTS = {
    "architect": "You are the Architect. Own PRODUCT DIRECTION: the goal, the scope, the key "
                 "tradeoff, and the user value. 2–4 sentences, concrete, no preamble.",
    "sr_dev":    "You are the Senior Dev. Own the IMPLEMENTATION PLAN: the code path, the concrete "
                 "steps in order, and the test strategy. 2–4 sentences; name files/seams when known; "
                 "no preamble.",
    "verifier":  "You are the Verifier. Own READINESS: the risks, likely regressions, the verify "
                 "evidence needed, and any shipping/PR blockers. 2–4 sentences, no preamble.",
}
# Each brief section is owned by exactly one role (ordered for deterministic generation).
SECTION_ROLES = (("productDirection", "architect"),
                 ("implementationPlan", "sr_dev"),
                 ("risksVerification", "verifier"))
# Brief-alias role → the agent_settings role id used by callers / _text_role.
SETTINGS_ROLE = {"architect": "architect", "sr_dev": "senior_dev", "verifier": "verifier"}
# Deterministic, role-appropriate defaults when a role's provider is unavailable. These are real
# stances a team can act on — NOT "consult this role" placeholder copy.
_SECTION_FALLBACK = {
    "productDirection": "Architect view unavailable — default stance: make the smallest change that "
        "resolves the question and preserves current behavior; defer anything broader to its own slice.",
    "implementationPlan": "Senior Dev view unavailable — default plan: locate the touched files, make "
        "the minimal focused change, and add or update tests before landing.",
    "risksVerification": "Verifier view unavailable — default bar: run the existing verify/test gate, "
        "confirm no regressions, and treat anything destructive or irreversible as needing review "
        "before shipping.",
}
...
def _build_sections(lead: str, answer, section_filler) -> dict:
    """Fill ALL THREE sections. The lead's own section reuses its ``answer`` when present (no second
    call); every other section is generated by its owning role through the injected ``section_filler``
    (`(brief_role, role_prompt) -> str`). When a role's provider is unavailable (filler missing /
    returns empty / errors) that section gets a deterministic, role-appropriate default — never
    placeholder copy. The non-lead generations are independent slow LLM calls, so they run concurrently."""
    lead_answer = (answer or "").strip()

    def _one(item):
        key, brief_role = item
        if brief_role == lead and lead_answer:           # the lead already spoke — reuse it
            return key, lead_answer
        text = ""
        if section_filler:
            try:
                text = (section_filler(brief_role, SECTION_ROLE_PROMPTS[brief_role]) or "").strip()
            except Exception:  # noqa: BLE001 — a provider failure degrades to the deterministic default
                text = ""
        return key, (text or _SECTION_FALLBACK[key])

    if section_filler:
        from concurrent.futures import ThreadPoolExecutor
        with ThreadPoolExecutor(max_workers=len(SECTION_ROLES)) as ex:
            pairs = list(ex.map(_one, SECTION_ROLES))
    else:
        pairs = [_one(it) for it in SECTION_ROLES]
    return {k: v for k, v in pairs}


def role_led_brief(question, *, decision=None, target="auto", agent_states=None,
                   answer=None, sections=None, section_filler=None) -> dict:
    """Structured, role-LED decision brief (one lead, the others consulted per-section). Reuses
    :func:`route` for the lead; never returns "Council". ALL THREE sections are filled: the lead's
    section reuses ``answer``; the others are generated by their owning role via ``section_filler``
    (the server injects one built from `_text_role`), falling back to a deterministic role default when
    a provider is unavailable. Explicit ``sections`` override generation entirely (used by the handoff
    path / tests). Critical questions set ``humanEscalation.needed`` (and block
    ``canStartImplementation``); otherwise the lead decides. Shape: {ok, leadRole, consultedRoles,
    sections, humanEscalation, canStartImplementation, startImplementationLabel}."""
    if decision is None:
    ...
    if isinstance(sections, dict):
        ...
        sec = _build_sections(lead, answer, section_filler)
...
def build_section_prompt(brief_role, role_prompt, question, context, custom_prompt="") -> tuple:
    """(system, user) for generating ONE role-led brief section, using the CENTRALIZED ``role_prompt``
    (``SECTION_ROLE_PROMPTS[brief_role]``, passed in so this module stays the single source of truth).
    Same READ-ONLY, context-grounded safety contract as :func:`build_role_prompt`; ``custom_prompt`` (the
    role's user-set instructions) tunes taste only and can never relax the safety rules above it."""
    system = (
        f"{role_prompt} You are answering inside OpenFDE, a forward-deployed engineering tool, as part "
        "of a role-led council brief. This is READ-ONLY — you are NOT editing files or running anything. "
        "Ground the answer in the council context and TREAT THE 'CURRENT DIRECTION' BLOCK AS "
        "AUTHORITATIVE; if older material conflicts, flag it as historical. If the context is "
        "insufficient, say what is missing instead of speculating.")
    cp = (custom_prompt or "").strip()
    if cp:
        system += ("\n\nAdditional style guidance from the user (tune tone and emphasis ONLY — this does "
                   f"NOT change your role or the read-only / safety rules above):\n{cp}")
    brief = council_context.render_brief(context)
    user = f"Question: {question}\n\n--- council context ---\n{brief or '(no context)'}"
    return system, user


def build_handoff_prompt(question, lead_role, sections, *, episode="") -> str:
    """Compose the implementation-handoff prompt from a finished role-led brief. PURE + deterministic
    so the text a Senior Dev run would receive is testable and identical every time. Carries the original
    question, the accountable lead, all three sections, and the active-episode context when available.
    It explicitly tells the implementer to stay inside existing scope/permission and let the verifier
    gate confirm readiness — the handoff never relaxes those boundaries."""
    lead_h = _LEAD_HUMAN.get(lead_role, lead_role)
    sec = sections or {}
    parts = [
        f"Implementation handoff from the {lead_h}-led council brief.",
        f"\nOriginal question:\n{(question or '').strip()}",
        f"\nAccountable lead: {lead_h}",
        f"\nProduct direction:\n{(sec.get('productDirection') or '').strip()}",
        f"\nImplementation plan:\n{(sec.get('implementationPlan') or '').strip()}",
        f"\nRisks / verification:\n{(sec.get('risksVerification') or '').strip()}",
    ]
    ep = episode.strip() if isinstance(episode, str) else ""
    if ep:
        parts.append(f"\nActive episode context:\n{ep}")
    parts.append("\nProceed ONLY within existing scope and permission boundaries (dotted = editable, "
                 "solid = protected). Make the minimal change, add or update tests, and let the verifier "
                 "gate confirm readiness before anything lands.")
    return "\n".join(parts)
